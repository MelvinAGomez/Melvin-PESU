...
# Tsunami Test
def send_request_tsunami(url, delay, no_of_requests):
...
def index():
    ...
    for msg in consumer_2:
        ...
        if test_type == "TSUNAMI":
            ...
            send_request_tsunami(url,delay_between_requests,number_of_requests)
        ...
        mode_latency = mode_latency = statistics.mode(latencies) if latencies else 0
        ...
        print(f"Mode Latency: {mode_latency} seconds")

        
        metrics[NODE_ID] = {"Test_ID":test_id,"Mean_Latency": mean_latency, "Median_Latency": median_latency, "Min_Latency": min_latency,"Max_Latency": max_latency,"Mode Latency":mode_latency}
